...
export default function Home() {
  const { playlistData, setPlaylistData, token, setToken, playlistId } = useContext(DataContext)
  ...
  const letterClickHandle = selection => {
    ...
    if (selection === playlistData[currentIndex].track[answerCriteria].split('')[0]) {
    ...
    <div className="min-h-screen bg-gray-600 text-white">
      <div className="fixed -z-1 inset-0 h-screen w-screen flex items-center justify-center">
        <div className="h-[400px] w-[400px] relative">
          <Lottie options={defaultOptions} height={400} width={400} isPaused={animationPaused} />
          {/* <img
      ...
      <div className="flex items-center justify-center relative">
        <audio src={playlistData[currentIndex]?.track?.preview_url} ref={audioRef} onEnded={() => recordScratchAudio.current.play()} />
        <audio src="/audio/recordScratch.mp3" ref={recordScratchAudio} onEnded={() => setAnimationPaused(true)} />

        <div className="grid grid-cols-4 md:grid-cols-6 w-full gap-5 p-5">
          {alphabet.split('').map(letter => (
            <button key={letter} onClick={() => letterClickHandle(letter)} className="aspect-square border border-white rounded-md hover:bg-slate-800 md:text-5xl bg-blue-900/20">
              {letter}
            </button>
          ))}
        </div>
      </div>
      <ModalOverlay isOpen={showFinalScreen} openModal={showFinalScreen} closeModal={() => {}}>
        <span className="text-2xl">
          You scored: {runningTotal}/{playlistData.length}
        </span>
        <ul>
          {userAnswers.map(answer => (
            <li key={answer.name} className="flex justify-between items-center my-5 last:mb-0 pt-5 border-t border-slate-300">
              ...
              <img className="flex-shrink-0 ml-2 w-[64px] h-[64px]" src={answer.album.images[2].url} />
            </li>
          ))}
        </ul>
        <a href="https://guess-that-song.vercel.app/dashboard" className="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 mt-2">
          Dashboard
        </a>
      </ModalOverlay>
      <ModalOverlay
      ...
      >
        {currentSong && (
          <>
            <img src={currentSong?.album?.images[1]?.url} className="w-[150px] h-[150px] shadow-md mb-5 mx-auto" />
            <h2 className="text-2xl">{currentSong?.name}</h2>

            <ul className="flex flex-wrap mt-3 justify-center">
              {currentSong?.artists?.map(artist => (
                <li className="text-sm mx-2 bg-pink-500 rounded-full px-3 py-1 mb-2 text-white" key={artist.name}>
                  {artist.name}
                </li>
              ))}
            </ul>
        ...
        >
          {currentIndex + 1 < playlistData.length ? <span>Next song</span> : <span>See results</span>}
        </button>
      </ModalOverlay>
      <ModalOverlay isOpen={!gameInProgress && !loading} openModal={!gameInProgress} closeModal={() => setGameInProgress(true)}>
      ...
      <ModalOverlay isOpen={showCreateModal} openModal={showCreateModal} closeModal={() => setShowCreateModal(false)}>
        ...
        {playlistPreview && (
          <div>
            <div className="mt-5 bg-slate-800 text-left p-3 flex rounded-lg items-center">
              <img className="w-[75px] h-[75px]" src={playlistPreview.images[0].url} alt="" />
              <span className="ml-5 text-xl">{playlistPreview.name}</span>
            </div>
            <CopyToClipboard text={`${window.location.origin}/?id=${userPlaylistId}&limit=${numberOfSongs}`} onCopy={() => setCopied(true)}>
      ...
      <Transition show={loading} enter="transition-opacity duration-75" enterFrom="opacity-0" enterTo="opacity-100" leave="transition-opacity duration-150" leaveFrom="opacity-100" leaveTo="opacity-0">
