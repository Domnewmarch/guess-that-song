...
import { BellIcon, ClockIcon, CogIcon, CreditCardIcon, DocumentReportIcon, HomeIcon, MenuAlt1Icon, QuestionMarkCircleIcon, ScaleIcon, MusicNoteIcon, ShieldCheckIcon, UserGroupIcon, XIcon } from '@heroicons/react/outline'
...
const navigation = [
  ...
  { name: 'Playlists', href: '#', icon: MusicNoteIcon, current: false },
  { name: 'Friends', href: '#', icon: UserGroupIcon, current: false },
  { name: 'Leaderboards', href: '#', icon: UserGroupIcon, current: false }
]
...
const cards = [
  ...
  { name: 'Bruce springsteen', href: 'https://guess-that-song.vercel.app/?id=37i9dQZF1DZ06evO1S7maQ', icon: MusicNoteIcon, amount: 'Heres brucey ' }, 
  { name: 'Bruce springsteen', href: 'https://guess-that-song.vercel.app/?id=37i9dQZF1DZ06evO0ENBD2', icon: MusicNoteIcon, amount: 'This is queen ' }, 
  // More items...
]
// recently activity
const playlists = [
...
const statusStyles = {
  ...
  Dreadful: 'bg-red-500 text-white'
}
      ...
      <div className="min-h-full">
                ...
                <Dialog.Panel className="relative flex-1 flex flex-col max-w-xs w-full pt-5 pb-4 bg-cyan-700">
                  ...
                  <nav className="mt-5 flex-shrink-0 h-full divide-y divide-cyan-800 overflow-y-auto" aria-label="Sidebar">
                    <div className="px-2 space-y-1">
                      {navigation.map(item => (
                        <a key={item.name} href={item.href} className={classNames(item.current ? 'bg-cyan-800 text-white' : 'text-cyan-100 hover:text-white hover:bg-cyan-600', 'group flex items-center px-2 py-2 text-base font-medium rounded-md')} aria-current={item.current ? 'page' : undefined}>
        ...
        <div className="hidden lg:flex lg:w-64 lg:flex-col lg:fixed lg:inset-y-0">
          ...
          <div className="flex flex-col flex-grow bg-cyan-700 pt-5 pb-4 overflow-y-auto">
            ...
            <nav className="mt-5 flex-1 flex flex-col divide-y divide-cyan-800 overflow-y-auto" aria-label="Sidebar">
              <div className="px-2 space-y-1">
                {navigation.map(item => (
                  <a key={item.name} href={item.href} className={classNames(item.current ? 'bg-cyan-800 text-white' : 'text-cyan-100 hover:text-white hover:bg-cyan-600', 'group flex items-center px-2 py-2 text-sm leading-6 font-medium rounded-md')} aria-current={item.current ? 'page' : undefined}>
                    <item.icon className="mr-4 flex-shrink-0 h-6 w-6 text-cyan-200" aria-hidden="true" />
                    {item.name}
                  </a>
                ))}
        ...
        <div className="lg:pl-64 flex flex-col flex-1">
          <div className="relative z-10 flex-shrink-0 flex h-16 bg-white border-b border-gray-200 lg:border-none">
            <button type="button" className="px-4 border-r border-gray-200 text-gray-400 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-cyan-500 lg:hidden" onClick={() => setSidebarOpen(true)}>
              ...
              <div className="flex-1 flex">
                <form className="w-full flex md:ml-0" action="#" method="GET">
                  ...
                  <div className="relative w-full text-gray-400 focus-within:text-gray-600">
                    ...
                    <input id="search-field" name="search-field" className="block w-full h-full pl-8 pr-3 py-2 border-transparent text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-0 focus:border-transparent sm:text-sm" placeholder="Search playlists" type="search" />
                  </div>
                </form>
              </div>
                ...
                <Menu as="div" className="ml-3 relative">
                  <div>
                    <Menu.Button className="max-w-xs bg-white rounded-full flex items-center text-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 lg:p-2 lg:rounded-md lg:hover:bg-gray-50">
                      <img className="h-8 w-8 rounded-full" src="/img/profile.png" alt="" />
                      <span className="hidden ml-3 text-gray-700 text-sm font-medium lg:block">
                        <span className="sr-only">Open user menu for </span>Dom Newmarch
                      </span>
          ...
          <main className="flex-1 pb-8">
                ...
                <div className="py-6 md:flex md:items-center md:justify-between lg:border-t lg:border-gray-200">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center">
                      <img className="hidden h-16 w-16 rounded-full sm:block" src="/img/profile.png" alt="" />
                      <div>
                        <div className="flex items-center">
                          <img className="h-16 w-16 rounded-full sm:hidden" src="/img/profile.png" alt="" />
                          <h1 className="ml-3 text-2xl font-bold leading-7 text-gray-900 sm:leading-9 sm:truncate">Hello, Dom Newmarch!</h1>
                        </div>
            ...
            <div className="mt-8">
              <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                <h2 className="text-lg leading-6 font-medium text-gray-900">Playlists</h2>
                        ...
                        <div className="flex items-center">
                          ...
                          <div className="ml-5 w-0 flex-1">
                            <dl>
                              ...
                              <dd>
                                <div className="text-md font-medium text-gray-900">{card.amount}</div>
                              </dd>
                            </dl>
                          </div>
                        </div>
              ...
              <div className="shadow sm:hidden">
                <ul role="list" className="mt-2 divide-y divide-gray-200 overflow-hidden shadow sm:hidden">
                  {playlists.map(playlist => (
                    <li key={playlist.id}>
                      <a href={playlist.href} className="block px-4 py-4 bg-white hover:bg-gray-50">
                        <span className="flex items-center space-x-4">
                          <span className="flex-1 flex space-x-2 truncate">
                            ...
                            <span className="flex flex-col text-gray-500 text-sm truncate">
                              ...
                              <span>
                                <span className="text-gray-900 font-medium">{playlist.amount}</span> {playlist.currency}
                              </span>
                              <time dateTime={playlist.datetime}>{playlist.date}</time>
                            </span>
                          </span>
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead>
                          <tr>
                            ...
                            <th className="hidden px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider md:block" scope="col">
                              Status
                            </th>
                            <th className="px-6 py-3 bg-gray-50 text-right text-xs font-medium text-gray-500 uppercase tracking-wider" scope="col">
                              Date
                          ...
                          {playlists.map(playlist => (
                            <tr key={playlist.id} className="bg-white">
                              <td className="max-w-0 w-full px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <div className="flex">
                                  <a href={playlist.href} className="group inline-flex space-x-2 truncate text-sm">
                                    <CashIcon className="flex-shrink-0 h-5 w-5 text-gray-400 group-hover:text-gray-500" aria-hidden="true" />
                                    <p className="text-gray-500 truncate group-hover:text-gray-900">{playlist.name}</p>
                                  </a>
                                </div>
                              </td>
                              <td className="px-6 py-4 text-right whitespace-nowrap text-sm text-gray-500">
                                <span className="text-gray-900 font-medium">{playlist.amount} </span>
                                {playlist.currency}
                              </td>
                              <td className="hidden px-6 py-4 whitespace-nowrap text-sm text-gray-500 md:block">
                                <span className={classNames(statusStyles[playlist.status], 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize')}>{playlist.status}</span>
                              </td>
                              <td className="px-6 py-4 text-right whitespace-nowrap text-sm text-gray-500">
                                <time dateTime={playlist.datetime}>{playlist.date}</time>
                              </td>
